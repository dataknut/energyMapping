...
ggplot2::ggplot(t_diff, aes(x = pc_diff)) +
  geom_histogram()
```
####Plotting the change in dom elec consumption against the deprevation score
```{r Loading in the Deprevation score data}
...
ggplot(Solent,aes(x =as.factor(IMD_Decile)), y =(de_diff/de2010), colour =as.factor(IMD_Decile), group =IMD_Decile) +  geom_boxplot()
