...
merged_de<-merge(domelec_LSOA_2010, domelec_LSOA_2019, by="Middle Layer Super Output Area (MSOA) Code", all=TRUE)
names(merged_de)
merged_de$de_diff<-as.numeric(merged_de$de2019)-as.numeric(merged_de$de2010)##BROKEN
summary(merged_de)
```

```{r Mapping the LSOA difference in mean dom elec consumption}
merged_de$LSOA11CD<-merged_de$"Middle Layer Super Output Area (MSOA) Code"
lsoa_de20102019<-merge(lsoa_sf_data, merged_de)
ggplot2::ggplot(lsoa_de20102019)+geom_sf(aes(fill="de_diff")+scale_fill_continuous(name="Change in Mean comsumpton (kWh per meter)"),low="green",high="red")+labs(caption="Solent(all LSOAs)")
##BROKEN
```
...
#Cities such as Southampton disappear due to their density, we will now map Southampton alone to see this area in more detail
mapData <- dplyr::filter(merged_de, LAD11NM == "Southampton")
# plotting the electricity consumption of Southampton in more detail
ggplot2::ggplot(mapData) + 
