...
names(LSOAdepelec)
df <- dplyr::select(LSOAdepelec, LSOA11CD, "Local Authority Name.x", "de_diff", IMDScore, de2010, IMD_Decile, de2019
)
...
nrow(Solent)
```
####Plotting the relationship between Electricty consumption and deprevation
...
lsoa_dgas20102019$pc_diff <- 100*(lsoa_dgas20102019$dg_diff/lsoa_dgas20102019$dgas2010)
t_diff <-  dplyr::select(as.data.frame(lsoa_dgas20102019), 
                         LSOA11CD, `Lower Layer Super Output Area (LSOA) Code`,
                         dgas2010, dgas2019, dg_diff, pc_diff)
head(arrange(t_diff, pc_diff))
head(arrange(t_diff, -pc_diff))
ggplot2::ggplot(t_diff, aes(x = pc_diff)) +
  geom_histogram()
...
```
####Plotting the change in gas usage against the deprevation 
```{r plotting change in gas usage against dep}
LSOAdepgas <- merge(merged_dgas, LSOAdep, by= "LSOA11CD")
names(LSOAdepgas)
df <- dplyr::select(LSOAdepgas, LSOA11CD, "Local Authority Name.x", dg_diff, IMDScore, dgas2010, IMD_Decile, dgas2019
)
...
# or without colour but using facets for Local Authorities
Solentex <- filter(Solent,100*(dg_diff/dgas2010)<50)
ggplot(Solentex, aes(x = as.factor(IMD_Decile), # forces a box plot at each value 
                   y =100*(dg_diff/dgas2010), 
                   group = IMD_Decile)) +
  geom_boxplot() +
  labs(x = "IMD Decile",
...
nrow(Solent)
```
#####Plotting the gas usage against deprevation
...
```
####Map with overlay 
```{r Check leaflet for change in dom gas consumption}
library(leaflet)
