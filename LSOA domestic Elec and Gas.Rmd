...
ggplot(Solent, aes(x = mean_de2010, # forces a box plot at each value 
                   y =mean_de_diff,
                   colour = la_name)) +
  geom_point() +
  geom_smooth() +
...
#names(LSOAdepgas)
df <- dplyr::select(LSOAdepgas, LSOA11CD, la_name.x, mean_dg2019, IMDScore, mean_dg2010, IMD_Decile,dg_diff
)
...
Try adding facets for each Local Authority and remove the outlier

```{r addFacetsTogasDiff}
# or without colour but using facets for Local Authorities
Solentex <- filter(Solent,100*(dg_diff/mean_dg2010)<50)
