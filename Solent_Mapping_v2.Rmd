...
###2021 Proportion of Population
```{r Loading in the 2021 Local Authority Fuel Poverty Data}
inFile<-paste0(dataFolder, "Fuel Poverty/2021 LA_Fuel_Poverty.csv")
la_fuelpov_2021<-readr::read_csv(inFile)
head(la_fuelpov_2021)
```
```{r Mapping Local Authority Fuel Poverty 2021}
la_fuelpov_2021$lad18cd <- la_Transport$"Area Codes" # Creating a variable with the local authority code in the same name 
...
ggplot2::ggplot(la_merged_Fuelpov2021) +
  geom_sf(aes(fill = "Proportion of households fuel poor (%)")) +
...
ggplot2::ggplot(lsoa_fuelpov_2021_merged) +
  geom_sf(aes(fill="Proportion of households fuel poor (%)"))+ # which data set has the scores?
