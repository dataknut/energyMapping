...
```
### Difference in proportion Fuel Poverty 2012 to 2018 (BROKEN)
...
names(merged_dfla)
merged_dfla$pc_diff <-merged_dfla$pc2018la - merged_df$pc2012la
```
```{r Mapping the difference in fuel poverty from 2012 to 2018}
merged_dfla$lad18cd <- merged_dfla$`LA Code` # Creating a variable with the local authority code in the same name 
la_merged_Fuelpov20182012dif <- merge(la_sf_data,merged_dfla)# Merge the local Authority boundaries and the Fuel Poverty data
la_merged_Fuelpov20182012dif$pcFP <- as.numeric(la_merged_Fuelpov20182012dif$`Proportion of households fuel poor (%)`) # convert to numeric from char - why does it load as char??
ggplot2::ggplot(la_merged_Fuelpov20182012dif) +
  geom_sf(aes(fill = pc_diffla)) +
