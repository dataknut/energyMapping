...
df$propFP <- as.numeric(substring(df$`Proportion Over 65`, 
                                  1, 
                                  nchar(df$`Proportion Over 65`)-1)
                        )
head(df)
# select the places we want here
ggplot(df, aes(x=propFP, y=`Proportion of households fuel poor (%)`)) +
  geom_point()

census_age$oldpop <- Census_age_fp$'Over 65s'/Census_age_fp$'Age: All usual residents; measures: Value'
ggplot(Census_age_fp, aes(x=oldpop, y=`Proportion of households fuel poor (%)`)) +
  geom_point()
```
